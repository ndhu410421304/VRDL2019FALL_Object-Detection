...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    print(ymin, xmin, ymax, xmax, box_to_display_str_map[box])
    if keypoints is not None:
...
def only_result(
  ...
  for box, color in box_to_color_map.items():
    ymin, xmin, ymax, xmax = box
    label_acc = str(box_to_display_str_map[box])
    label_acc = label_acc.strip('[')
    label_acc = label_acc.strip(']')
    label_acc = label_acc.strip("'")
    label_acc = label_acc.strip('num')
    label_acc = label_acc.strip('%')
    qlabel, qacc = label_acc.split(': ')
    #print(ymin, xmin, ymax, xmax, qlabel, qacc)
    bbox_arr = []
    bbox_arr.append(ymin)
    bbox_arr.append(xmin)
    bbox_arr.append(ymax)
    bbox_arr.append(xmax)
    b_bbox_arr.append(bbox_arr)
    qlabel_arr.append(qlabel)
    qacc_arr.append(qacc)
  return b_bbox_arr, qlabel_arr, qacc_arr
